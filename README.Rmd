...
```{r}
## Build panel and estimate test performance
ensembleMod <- amritr::ensembleEnet(X.train, Y.train, alpha=1, lambda=NULL, X.test, Y.test)
ensembleResult <- ensembleMod$result %>% zip_nPure()
ensemblePanel <- ensembleResult$enet.panel
ensemblePanel

## Estimate panel performance using cross-validation
ensembleTrain <- amritr::perfEnsemble(object=ensembleMod, validation = "Mfold", M = M, iter = iter, threads = cpus, progressBar = TRUE)
