...
#' @export
## Random Forest
supportVectorMachine = function(X, Y, X.test=NULL, Y.test=NULL){
  library(e1071)

  # Run random forest classifier
  colnames(X) <- paste("Feature", 1:ncol(X), sep=".")
  y.train0 <- Y[rownames(X)]
  fit = svm(y.train0 ~ ., data=X, importance=TRUE, proximity=TRUE)
...
#' @param direction = "auto", ">", "<"
#' @export
svmCV = function(X, Y, M, folds, progressBar){
  ...
  for (i in 1:M) {
    ...
    svm.res = suppressWarnings(supportVectorMachine(X=X.train, Y=Y.train))
    probs[[i]] <- predict(svm.res$fit, X.test, type='response')
    predictResponseList[[i]] <- predict(svm.res$fit, X.test, type='class')
  }
  ...
  threads = 4, progressBar = TRUE)
{
  ...
  if (validation == "Mfold") {
    ...
    parallel::clusterExport(cl, varlist=c("svmCV", "supportVectorMachine", "X", "Y", "M", "folds", "progressBar"), envir=environment())
    ...
    M = n
    cv <- svmCV(X, Y, M, folds, progressBar)
  }
