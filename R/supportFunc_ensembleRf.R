...
#' @export
ensembleRf = function(X.train, Y.train, X.test, Y.test){
  result <- mapply(function(X.train, X.test){
    rforest(X.train, Y.train, X.test=X.test, Y.test=Y.test)
...
#' @param direction = "auto", ">", "<"
#' @export
ensembleRfCV = function(X, Y, M, folds, progressBar){
  J <- length(X)
  ...
  for (i in 1:M) {
    ...
    result <- mapply(function(X.train, X.test){
      rforest(X.train, Y.training[[i]], X.test=X.test, Y.test=Y.test[[i]])
...
#' @param direction = "auto", ">", "<"
#' @export
perfEnsembleRf = function(object, validation = "Mfold", M = M, iter = iter, threads = threads, progressBar = TRUE){
  ...
  if (validation == "Mfold") {
    ...
    parallel::clusterExport(cl, varlist=c("ensembleRfCV", "ensembleRf", "rforest", "X", "Y", "M", "folds", "progressBar"), envir=environment())
    cv <- parallel::parLapply(cl, folds, function(foldsi, X, Y, M, progressBar){
      ensembleRfCV(X=X, Y=Y, M=M, folds = foldsi, progressBar=progressBar)
    ...
    M = n
    cv <- ensembleRfCV(X, Y, alpha, M, folds, progressBar)
  }
