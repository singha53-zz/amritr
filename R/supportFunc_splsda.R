...
perf.tuned.splsda = function(object, validation = "Mfold", M = 5, iter = 5, progressBar = TRUE){
  ...
  if (validation == "Mfold") {
    ...
    for(i in 1 : length(folds)){
      cvList[[i]] <- tuned.spsldaCV(X = X, Y = Y, folds = folds[[i]], progressBar = progressBar,
        ncomp = ncomp, keepXgrid = keepXgrid, validation = validation, M = M)
    }
    cv <- cvList %>% amritr::zip_nPure()
    errRes <- cv$errorRate %>% amritr::zip_nPure() %>% lapply(., function(j){
      do.call(rbind, j)
    })
    errorRate <- do.call(rbind, errRes) %>% as.data.frame %>%
      ...
      dplyr::group_by(Index, Method) %>% dplyr::summarise(Mean = mean(Value), SD = sd(Value)) %>%
      ungroup %>% dplyr::group_by(Method) %>%
      dplyr::filter(Index == "BER")
    aucRes <- cv$auc %>% amritr::zip_nPure() %>% lapply(., function(j){
      do.call(rbind, j)
    })
    auc <- do.call(rbind, aucRes) %>% as.data.frame %>%
      mutate(Method = rep(names(errRes), each = nrow(errRes[[1]]))) %>%
      tidyr::gather(Index, Value, -Method) %>%
      dplyr::group_by(Index, Method) %>% dplyr::summarise(Mean = mean(Value), SD = sd(Value)) %>%
      ungroup
    ...
    M = n
    cv <- tuned.spsldaCV(X = X, Y = Y, folds = foldsi, progressBar = progressBar,
      ncomp = ncomp, keepXgrid = keepXgrid, validation = validation, M = M)
...
tuned.spsldaCV = function(X = X, Y = Y, ncomp = ncomp, folds = folds, progressBar = FALSE,
  ...
  for (i in 1:M) {
    ...
    spls.res0 = tuned.spslda(X.train, Y.train, ncomp, keepXgrid, validation, M)

    testErr <- lapply(spls.res0$err$Method, function(z){
      keepX <- rep(subset(spls.res0$err, Method == z)$keepX, ncomp)
      spls.res = mixOmics::splsda(X.train, Y.train, ncomp, keepX, mode = "regression")
      if (!is.null(spls.res$nzv$Position))
        X.test = X.test[, -spls.res$nzv$Position]
      predictResponseList <- predict(object = spls.res, newdata = X.test, dist = paste(z, "dist", sep="."))$class
      probs <- predict(object = spls.res, newdata = X.test, dist = paste(z, "dist", sep="."))$predict
      return(list(predictResponseList = predictResponseList, probs = probs))
    }) %>% zip_nPure()
    predictResponseList[[i]] <- testErr$predictResponseList
    probs[[i]] <- testErr$probs
  }

  errorRate <- predictResponseList %>% zip_nPure() %>% lapply(., function(i){
    predictedLabels <- do.call(rbind, lapply(i, function(j) j[[1]]))[, paste("comp", ncomp, sep = " ")]
